...
class CounterfactualGenerator:
    ...
    def get_imputed_inputs(
        self, input: pd.DataFrame, n: int, col_names_with_missing_values: list[str]
    ) -> pd.DataFrame:
    ...
    def generate_explanations(self, input: pd.DataFrame, n: int) -> pd.DataFrame:
        ...
        if len(col_names_with_missing_values) > 0:
            inputs_to_explain = self.get_imputed_inputs(
                input, n, col_names_with_missing_values
            )
        else:
            inputs_to_explain = pd.DataFrame(
                np.repeat(input.values, n, axis=0), columns=input.columns
            )
